...
(function() {
	...
	/*窗口添加按钮*/
        $(document).on('click', '#calendar_confirm_btn', function () {
            if(setQuanx()) {
                ...
                var date0 = $("#cen_year").text() + "-" + start_date.split("-")[0];
                if (start_date.split("-")[1] < end_date.split("-")[1]) {
                    for (var i = parseInt(start_date.split("-")[1]); i <= parseInt(end_date.split("-")[1]); i++) {
                        $(".fullYearPicker .picker .month-container:eq(" + (parseInt(start_date.split("-")[0]) - 1) + ") td").each(function () {
                            var z = i + "";
                            if (z.length === 1) {
                                z = "0" + z
                            }
                            if (($(this).text().length === 1 ? ("0" + $(this).text()) : $(this).text()) === z) {
                                var leng = queryCalendar(date0 + "-" + z, date0 + "-" + z);
                                console.log(date0 + "-" + z)
                                console.log(leng)
                                if (leng.length > 0) {
                                    delCalendar(leng.id)
                                    $(this).removeClass("tradeday")
                                    $(this).removeClass("freeday")
                                    return;
                                } else {
                                    insertCalendar($("#cen_year").text() + "-" + start_date.split("-")[0] + "-" + i)
                                }
                            }
                        });
                    }
                } else {
                    $(".fullYearPicker .picker .month-container:eq(" + (parseInt($("#hd-start-date").val().split("-")[1]) - 1) + ") td").each(function () {
                        if (($(this).text().length === 1 ? ("0" + $(this).text()) : $(this).text()) === $("#hd-start-date").val().split("-")[2]) {
                            var leng = queryCalendar($("#hd-start-date").val(), $("#hd-start-date").val());
                            if (leng.length > 0) {
                                if (confirm("确定删除么？")) {
                                    delCalendar(leng.id)
                                    console.log($(this))
                                ...
                                insertCalendar($("#hd-start-date").val());
                            }
                        }
                    });
                }
	...
	function open_modal(start_date,end_date){
		...
		var len=queryCalendar(start_date,end_date);
		if(len.length>0){
			$("#calendar_confirm_btn").html("删除");
		}else{
            $("#calendar_confirm_btn").html("添加");
		}
		//$("#calendar-modal-1").modal();
		$(".month-container .selected").removeClass("selected");
       window.location.href='/calendar2?year='+$("#cen_year").html()+"&month="+getMonth($(this).text()),'考勤日历';
	}
				...
				.each(
						function() {
							...
							me.append('<div class="year">'
													+'<table style="width: 150px;text-align: center;margin-left: 43%;">'
